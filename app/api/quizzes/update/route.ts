...
export async function PUT(request: NextRequest) {
  try {
    ...
    // Try to update quiz via backend API using Swagger endpoint
    try {
      ...
      if (response.ok) {
        ...
        console.error('Error updating quiz - Status:', response.status)
        console.error('Error updating quiz - Response:', errorText)
        console.error('Error updating quiz - Request body sent:', JSON.stringify(requestBody, null, 2))
        ...
        return NextResponse.json(
          { error: `Failed to update quiz: ${errorMessage}` },
      ...
      return NextResponse.json(
        { error: fetchError?.message || 'Failed to update quiz' },
        { status: 500 }
      )
    }
  } catch (error: any) {
